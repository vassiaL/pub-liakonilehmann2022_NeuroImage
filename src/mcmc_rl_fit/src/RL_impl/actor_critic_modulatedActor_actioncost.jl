...
and an extra free parameter as an action cost,
...
"""
function getLL_ActorCritic_TDLambda_modulatedActor_actioncost(data::Array{Int,2},
                                            alpha::Float64,
    ...
    if data_stats.nr_subjects != 1
        error("Invalid argument: getLL_ActorCritic_TDLambda_modulatedActor_actioncost supports only one subject")
    end
    ...
    for i_seed in 1:length(particlefilterseeds)
        ...
        for i = 1:nr_samples
            ...
            unbiased_p_sa = getSoftmaxProbs(learner.policyparameters[state,:], temp, [0., 0.])[action]
            ...
            V_state_seeds[i_seed][i] = learner.V[state]
