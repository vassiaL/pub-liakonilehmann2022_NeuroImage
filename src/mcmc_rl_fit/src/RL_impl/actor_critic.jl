...
This implementation follows Sutton & Barto, Chapter 7.7
...
"""
function getLL_ActorCritic_TDLambda(data::Array{Int,2}, alpha::Float64, eta::Float64,
    ...
    if data_stats.nr_subjects != 1
        error("Invalid argument: getLL_ActorCritic_TDLambda supports only one subject")
    end
    ...
    V_state = zeros(Float64, nr_samples + data_stats.nr_rewards) # V_state = zeros(Float64, nr_samples + data_stats.nr_rewards)
    ...
    for i = 1:nr_samples
        ...
        V_state[j] = learner.V[state]
        if done
            j+=1
            if j < (nr_samples + data_stats.nr_rewards)
                V_state[j] = learner.V[next_state] # Track V value of landing state in case it is the Goal
            end
        end
        j+=1
        ...
        TD_record[i] = learner.td_error[1]
